    ...
    unsigned long y )
{
    ...
    // O movimento do mouse nao pode alocar estrutura.
    // Talvez o ponteiro pode vir como argumento. 
    // Pois ainda nao temos um kfree() que funcione.
    // Precisamos usar um buffer interno para essa estrutura.
    
    // void *buffer;
    // buffer = (void*) kmalloc ( sizeof( struct bmp_header_d ) );
    
    char buffer[512];
    bh = (struct bmp_header_d *) &buffer[0];
    // bh = (struct bmp_header_d *) &arg_buffer[0];
    ...
    unsigned long y )
{
    ...
    // #todo: 
    // Use 'get system metrics' if possible.

    xLimit = SavedX;
    yLimit = SavedY;

    if ( xLimit==0 || yLimit==0 ){
        printf ("bmpDisplayBMP: Limits null \n");
        goto fail;
    }
...
//320   - 32 bpp (True color, RGBA)	
    ...
    // 8 bit
    // 8 bits pra cada cor, 256 cores, 1024 bytes.
    // 24bpp, 32bpp.
