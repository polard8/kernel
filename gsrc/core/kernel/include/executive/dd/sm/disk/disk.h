...
//Definições de tipos usados pelo Nelson.
...
int ide_read_sector(char nport,uint16_t count,uint64_t addr,void *buffer);
int ide_write_sector(char nport,uint16_t count,uint64_t addr,void *buffer);
