...
// Clear the list.
    ...
    for (count=0; count<CUBE_MAX; count++)
    {
        ...
        cube->vecs[1].x = (float) -0.8f;  cube->vecs[1].y = (float) -0.8f;  cube->vecs[1].z = (float) 0.8f;
        cube->vecs[2].x = (float)  0.8f;  cube->vecs[2].y = (float) -0.8f;  cube->vecs[2].z = (float) 0.8f;
        cube->vecs[3].x = (float) -0.8f;  cube->vecs[3].y = (float)  0.8f;  cube->vecs[3].z = (float) 0.8f;
        cube->vecs[4].x = (float)  0.8f;  cube->vecs[4].y = (float)  0.8f;  cube->vecs[4].z = (float) 0.8f;

        cube->vecs[5].x = (float) -0.8f;  cube->vecs[5].y = (float)  0.8f;  cube->vecs[5].z = (float) -0.8f;
        cube->vecs[6].x = (float)  0.8f;  cube->vecs[6].y = (float)  0.8f;  cube->vecs[6].z = (float) -0.8f;
        cube->vecs[7].x = (float) -0.8f;  cube->vecs[7].y = (float) -0.8f;  cube->vecs[7].z = (float) -0.8f;
        cube->vecs[8].x = (float)  0.8f;  cube->vecs[8].y = (float) -0.8f;  cube->vecs[8].z = (float) -0.8f;
...
// called by the engine
void paintFlyingCubeScene(void)
{
...
// Draw the bar.
// Isso pode ser um tipo de status bar,
// e uma flag determina se ela deve ou não aparecer
// no frame atual
    //if (show_yellow_bar)
    yellowstatus0(buf_fps,FALSE);

// O problema com a taskbar é que
// a rotina chamada aqui deve estar fazendo um refresh dos botoes
// em momento que não deveria.
    //wm_Update_TaskBar("Taskbar",FALSE);  // Cool.
