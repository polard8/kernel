...
void get_cpu_intel_parameters (void){
    ...
    debug_print ("detect-get_cpu_intel_parameters:\n");


    if ( (void *) processor == NULL ){
        panic ("detect-get_cpu_intel_parameters: processor\n");
    }
    ...
    // Intel?
    // #todo: Definir Intel como 1. Criar defines.
    ...
    // Checking the limit of feature id.

    if ( processor->MaxFeatureId < 6)
    {
        debug_print ("get_cpu_intel_parameters: MaxFeatureId limits\n");
...
// It was called by: init_architecture_dependent in init.c
