...
//executar o aplicativo indicado pelo argumento em main
//ou o gdeshell, caso contrário, executar o shell interno.
int _init_app = 1; 
//int _init_app = 0; 
 ...
 * @todo:
 *    +Checar argumentos.
 *    +Criar a janela do TERMINAL. 
 *    +Enviar mensagem de texto para ele.
...
// #importante
// Vamos construir um terminal em user mode.
// Mas existirá um shell interno.
 
// #importante
// O terminal vai inicializar o aplicativo indicado via argumento,
// se falhar então ele tenta inicializar o gdeshell, se falhar
// ele inicializa o shell interno.
...
noArgs:		
    ...
    // #test
    // Vamos diminuir para colocarmos um barra de ferramentas.
    // Os botões podem ajudar o desenvolvedor.
	...
	if ( _init_app == 1 )
	{
		...
		if ( pidFORK == 0 )
		{
		    ...
		    printf ("gdeterm: estamos no FILHO\n");
			printf ("PID=%d", getpid());
			
			execve ("gdeshell.bin", NULL, NULL );
			
			printf ("gdeterm: execve falhou.\n");
			goto do_run_internal_shell;
		}	
	};
	*/
