...
//#ifdef KERNEL_VERBOSE
    printf("sm-disk-disk-ata_initalize:\n");
...
//#endif
	...
	{
		// Error.
		printf("sm-disk-disk-ata_initalize: pci_scan_device fail. ret={%d} \n", (_u32) data );
		refresh_screen();
	...
	{
		// Error.
        printf("sm-disk-disk-ata_initalize: Error Driver [%X]\n",data);
        refresh_screen();
		return (int) 1;    
	
	}else if( data == PCI_MSG_AVALIABLE )
	      {
              printf("sm-disk-disk-ata_initalize: RAID Controller Not supported.\n");
		      refresh_screen();
              return (int) 1;       
          };
...
//#endif
              ...
              //ahci_mass_storage_init();
			   ...
			   printf("sm-disk-disk-ata_initalize: Panic!\n");
...
#ifdef KERNEL_VERBOSE	
	printf("ata_pci_configuration_space:\n");
              ...
              return PCI_MSG_AVALIABLE;
	                ...
	                ata_pci.subclasse == 6)
		        {
                    ...
                    //Ok.
					 ...
					 printf("sm-disk-disk-ata_pci_configuration_space: PANIC DRIVER BLOCK!");
		             die();
                 };
 ...
 * Khole OS v0.3
 *
 * Obs: O que segue são rotinas de suporte a IDE ATAPI.
 */
                       ...
                       void *buffer )
{
	...
	// #bugbug
	// ?? Não sei qual é o problema aqui.
	// FIXME
    ata_assert_dever(nport); 
    ...
    // Verifique se é a mesma unidade para não esperar.
	...
	//
	// @todo:
    // ?? Nelson é mesmo correcto DRQ aqui ??
	//
 ...
 *
 * Obs:
 *     O que segue são rotinas de suporte a IDE ATA.
 */
